...
async def change_how_long(msg: types.Message, state: FSMContext):
    await msg.answer(text="В течение какого времени Вы хотите снимать квартиру?", reply_markup=keyboard.how_long_keyboard)
...
async def change_budget(msg: types.Message, state: FSMContext):
    await msg.answer(text="В Каков Ваш бюджет для аренды квартиры?", reply_markup=keyboard.budget_keyboard)
...
async def change_location_global(msg: types.Message, state: FSMContext):
    await msg.answer(text="Где бы Вы хотели жить?", reply_markup=keyboard.location_global_keyboard)
...
async def change_location_local(msg: types.Message, state: FSMContext):
    await msg.answer(text="Где бы Вы хотели жить (уточн.)?", reply_markup=keyboard.location_local_keyboard)
...
async def change_apartment(msg: types.Message, state: FSMContext):
    await msg.answer(text="Нашли ли Вы уже квартиру?", reply_markup=keyboard.binary_keyboard)
...
async def change_pet(msg: types.Message, state: FSMContext):
    await msg.answer(text="Готовы ли Вы жить с соседом, у которого будет домашнее животное?", reply_markup=keyboard.binary_keyboard)
